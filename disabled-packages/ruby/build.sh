...
termux_step_make_install () {
	...
	local RBCONFIG=$TERMUX_DESTDIR/usr/lib/ruby/${_MAJOR_VERSION}.0/${TERMUX_HOST_PLATFORM}/rbconfig.rb
...
termux_step_post_massage () {
	if [ ! -f $TERMUX_DESTDIR/usr/lib/ruby/${_MAJOR_VERSION}.0/${TERMUX_HOST_PLATFORM}/readline.so ]; then
