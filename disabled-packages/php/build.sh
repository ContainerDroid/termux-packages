...
--enable-calendar
--enable-exif
--enable-gd-native-ttf=$TERMUX_DESTDIR/usr
--enable-mbstring
--enable-opcache
--enable-pcntl
--enable-sockets
...
--with-bz2=$TERMUX_DESTDIR/usr
--with-curl=$TERMUX_DESTDIR/usr
--with-freetype-dir=$TERMUX_DESTDIR/usr
--with-gd=$TERMUX_DESTDIR/usr
--with-iconv=$TERMUX_DESTDIR/usr
--with-libxml-dir=$TERMUX_DESTDIR/usr
--with-openssl=$TERMUX_DESTDIR/usr
--with-pcre-regex=$TERMUX_DESTDIR/usr
--with-png-dir=$TERMUX_DESTDIR/usr
--with-readline=$TERMUX_DESTDIR/usr
--with-zlib
--with-pgsql=shared,$TERMUX_DESTDIR/usr
--with-pdo-pgsql=shared,$TERMUX_DESTDIR/usr
--with-apxs2=$TERMUX_DESTDIR/usr/bin/apxs
--enable-fpm
...
termux_step_pre_configure () {
	...
	export EXTENSION_DIR=$TERMUX_DESTDIR/usr/lib/php
}
...
termux_step_post_make_install () {
	mkdir -p $TERMUX_DESTDIR/etc/php-fpm.d
	cp sapi/fpm/php-fpm.conf $TERMUX_DESTDIR/etc/
	cp sapi/fpm/www.conf $TERMUX_DESTDIR/etc/php-fpm.d/

	sed -i 's/SED=.*/SED=sed/' $TERMUX_DESTDIR/usr/bin/phpize
}
