...
TERMUX_PKG_REVISION=1
TERMUX_PKG_EXTRA_CONFIGURE_ARGS="--disable-dynamic-pcre --sysconfdir=$TERMUX_DESTDIR/etc/privoxy"
...
termux_step_post_make_install() {
    ...
    rm -f "$TERMUX_DESTDIR/usr/sbin/$STRIP"
}

termux_step_post_massage() {
    # copy default config files
    for f in $TERMUX_PKG_CONFFILES; do
        cp "$TERMUX_PKG_SRCDIR/$(basename $f)" "$TERMUX_PKG_MASSAGEDIR/$f"
    done
}
