...
TERMUX_PKG_EXTRA_MAKE_ARGS="tooldir=$TERMUX_DESTDIR/usr"
TERMUX_PKG_RM_AFTER_INSTALL="usr/share/man/man1/windmc.1 usr/share/man/man1/windres.1 usr/bin/ld.bfd"

# Avoid linking against libfl.so from flex if available:
export LEXLIB=

termux_step_post_make_install () {
	cp $TERMUX_PKG_BUILDER_DIR/ldd $TERMUX_DESTDIR/usr/bin/ldd
