...
termux_step_extract_into_massagedir () {
	mkdir -p $TERMUX_PKG_MASSAGEDIR/usr/lib/pkgconfig $TERMUX_PKG_MASSAGEDIR/usr/include
	cp -Rf $TERMUX_STANDALONE_TOOLCHAIN/sysroot/usr/include/* $TERMUX_PKG_MASSAGEDIR/usr/include
	local _LIBDIR=lib
	if [ "$TERMUX_ARCH" = "x86_64" ]; then
		_LIBDIR=lib64
	fi
	cp $TERMUX_STANDALONE_TOOLCHAIN/sysroot/usr/${_LIBDIR}/*.o $TERMUX_PKG_MASSAGEDIR/usr/lib

	cp "$PKG_CONFIG_LIBDIR/zlib.pc" $TERMUX_PKG_MASSAGEDIR/usr/lib/pkgconfig/zlib.pc
	...
	cp $LIBATOMIC_PATH $TERMUX_PKG_MASSAGEDIR/usr/lib/libatomic.a
	...
	cd $TERMUX_PKG_MASSAGEDIR/usr/lib
