...
termux_step_make_install () {
	...
	rm -Rf $TERMUX_DESTDIR/usr/lib/libbz2*
	rm -Rf $TERMUX_DESTDIR/usr/bin/{bzcat,bunzip2}
	cp bzip2-shared $TERMUX_DESTDIR/usr/bin/bzip2
	cp libbz2.so.${TERMUX_PKG_VERSION} $TERMUX_DESTDIR/usr/lib
	(cd $TERMUX_DESTDIR/usr/lib && ln -s libbz2.so.${TERMUX_PKG_VERSION} libbz2.so.1.0)
	(cd $TERMUX_DESTDIR/usr/lib && ln -s libbz2.so.${TERMUX_PKG_VERSION} libbz2.so)
	(cd $TERMUX_DESTDIR/usr/bin && ln -s bzip2 bzcat)
	(cd $TERMUX_DESTDIR/usr/bin && ln -s bzip2 bunzip2)
	# bzgrep should be enough so remove bz{e,f}grep
	rm $TERMUX_DESTDIR/usr/bin/bz{e,f}grep $TERMUX_DESTDIR/usr/share/man/man1/bz{e,f}grep.1
}
