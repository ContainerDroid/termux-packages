...
termux_step_make_install() {
	rm $TERMUX_DESTDIR/usr/lib/libboost* -f
	rm $TERMUX_DESTDIR/usr/include/boost -rf

	./bootstrap.sh

	echo "using clang : $TERMUX_ARCH : $CXX : <linkflags>-L${TERMUX_DESTDIR}/usr/lib ; " >> project-config.jam

	./b2 target-os=android -j${TERMUX_MAKE_PROCESSES} \
		include=$TERMUX_DESTDIR/usr/include \
